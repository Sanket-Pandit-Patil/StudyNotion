    ...
    <>
      ...
      >
        <div className="flex w-11/12 max-w-maxContent items-center justify-between">
                      ...
                      >
                        ...
                        <div className="invisible absolute left-[50%] top-[50%] z-[1000] flex w-[200px] translate-x-[-50%] translate-y-[3em] flex-col rounded-lg bg-richblack-5 p-4 text-richblack-900 opacity-0 transition-all duration-150 group-hover:visible group-hover:translate-y-[1.65em] group-hover:opacity-100 lg:w-[300px]">
                          ...
                          {loading ? (
                            ...
                            <>
                              {subLinks       //filter vala call remove kiya hai
                                ?.map((subLink, i) => (
                                  <Link
          ...
          {/* Mobile menu button */}
          <button 
            className="mr-4 md:hidden p-2 rounded-md hover:bg-richblack-700 transition-colors"
          ...
          >
            <div className="flex flex-col h-full">
              ...
              <div className="flex items-center justify-between p-4 border-b border-richblack-700">
                ...
                <button 
                  onClick={toggleMobileMenu}
                  className="p-2 rounded-md hover:bg-richblack-700 transition-colors"
                >
                    ...
                    <li key={index}>
                      {link.title === "Catalog" ? (
                        <div className="space-y-2">
                          ...
                          {loading ? (
                            <p className="text-sm text-richblack-400 pl-4">Loading...</p>
                          ) : subLinks.length ? (
                              ...
                              {subLinks?.map((subLink, i) => (
                                <Link
                                  to={`/catalog/${subLink.name
                                    ...
                                    .join("-")
                                    .toLowerCase()}`}
                                  className="block text-sm text-richblack-300 hover:text-yellow-25 transition-colors"
                                  key={i}
                                  onClick={toggleMobileMenu}
                                >
                                  {subLink.name}
                            ...
                            <p className="text-sm text-richblack-400 pl-4">No Courses Found</p>
                          )}
                        </div>
                      ) : (
              ...
              {/* Mobile auth section */}
              <div className="p-4 border-t border-richblack-700 space-y-4">
                {user && user?.accountType !== ACCOUNT_TYPE.INSTRUCTOR && (
                  <Link 
                    to="/dashboard/cart" 
                    className="flex items-center gap-2 text-richblack-100 hover:text-yellow-25 transition-colors"
                    onClick={toggleMobileMenu}
                  >
